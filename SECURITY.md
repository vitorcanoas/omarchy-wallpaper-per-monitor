...
- `bin/wallpaper-monitor` and `bin/wp` are run directly by the user (or a
  keybinding the user configures) to edit the override file below. Neither
  the shell nor any other plugin process ever invokes them.
- `bin/omarchy-wallpaper-render` is a standalone, opt-in render pipeline for
  preparing source art (see "Render pipeline" below). It is never invoked by
  `Background.qml`, `wallpaper-monitor`, or `wp`, and is not part of the
...
- `~/.local/bin/{wallpaper-monitor,wp,omarchy-wallpaper-render}` — symlinks
...
deliberate, not an oversight, and a reviewer should not ask for it to be
renamed.
...
- write through a temp file (`mktemp`) plus `mv` into place, rather than
  editing in place;
- merge with `jq`, reading and rewriting only the `plugins[]` and
  `disabledPlugins[]` arrays (deduplicated by plugin `id`), leaving every
  other field in `shell.json` untouched;
...
### Render pipeline (`bin/omarchy-wallpaper-render`)

This script is a separate, optional utility for preparing source art before
it is used as a wallpaper. It is not part of the plugin's runtime path — it
is never invoked by `Background.qml`, `wallpaper-monitor`, `wp`,
`install.sh`, or `uninstall.sh`. It shells out to two local binaries,
`realesrgan-ncnn-vulkan` and `magick` (or `convert` as a fallback), operating
only on local image files under the repository's own directory tree. It
makes no network requests either.
...
(`~/.local/bin/{wallpaper-monitor,wp,omarchy-wallpaper-render}`) are outside
